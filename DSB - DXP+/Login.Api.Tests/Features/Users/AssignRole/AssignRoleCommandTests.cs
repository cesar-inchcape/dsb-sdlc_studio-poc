...
namespace Login.Api.Tests.Features.Users.AssignRole;
...
public class AssignRoleCommandTests
{
    ...
    [Fact]
    public async Task Handle_WithValidData_AssignsRoleToUser()
    {
        // Arrange
        ...
        var roleToAssign = context.Roles.First(r => r.Name == "DistributorAdmin");
        ...
        result.RoleId.Should().Be(roleToAssign.Id);
        result.Message.Should().Contain("assigned to user");

        var userRole = context.UserRoles.FirstOrDefault(ur => ur.UserId == user.Id && ur.RoleId == roleToAssign.Id);
    ...
    [Fact]
    public async Task Handle_UserAlreadyHasRole_ThrowsException()
    {
        // Arrange
        var context = GetDbContext();
        var handler = new AssignRoleCommandHandler(context);
